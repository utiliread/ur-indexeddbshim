...
function escapeDatabaseNameForSQLAndFiles(db) {
    if (CFG_js_1.default.escapeDatabaseName) {
        ...
        return CFG_js_1.default.escapeDatabaseName(escapeSQLiteStatement(db));
    }
    db = 'D' + escapeNameForSQLiteIdentifier(db);
    if (CFG_js_1.default.escapeNFDForDatabaseNames !== false) {
        // ES6 copying of regex with different flags
        db = db.replace(new RegExp(unicode_regex_js_1.default, 'gu'), function (expandable) {
            return '^4' + expandable.codePointAt().toString(16).padStart(6, '0');
        });
    }
    if (CFG_js_1.default.databaseCharacterEscapeList !== false) {
        db = db.replace((CFG_js_1.default.databaseCharacterEscapeList
            ? new RegExp(CFG_js_1.default.databaseCharacterEscapeList, 'gu')
            : /[\u0000-\u001F\u007F"*/:<>?\\|]/gu), // eslint-disable-line no-control-regex
        function (n0) {
            return '^1' + n0.charCodeAt().toString(16).padStart(2, '0');
        });
    }
    if (CFG_js_1.default.databaseNameLengthLimit !== false &&
        db.length >= ((CFG_js_1.default.databaseNameLengthLimit || 254) - (CFG_js_1.default.addSQLiteExtension !== false ? 7 /* '.sqlite'.length */ : 0))) {
        throw new Error('Unexpectedly long database name supplied; length limit required for Node compatibility; passed length: ' +
            db.length + '; length limit setting: ' + (CFG_js_1.default.databaseNameLengthLimit || 254) + '.');
    }
    return db + (CFG_js_1.default.addSQLiteExtension !== false ? '.sqlite' : ''); // Shouldn't have quoting (do we even need NUL/case escaping here?)
}
...
function unescapeDatabaseNameForSQLAndFiles(db) {
    if (CFG_js_1.default.unescapeDatabaseName) {
        ...
        return CFG_js_1.default.unescapeDatabaseName(unescapeSQLiteResponse(db));
    }
...
function isIdentifier(item) {
    ...
    var UnicodeIDStart = CFG_js_1.default.UnicodeIDStart || '[$A-Z_a-z]';
    // ID_Continue (includes Other_ID_Continue)
    var UnicodeIDContinue = CFG_js_1.default.UnicodeIDContinue || '[$0-9A-Z_a-z]';
