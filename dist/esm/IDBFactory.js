...
var hasNullOrigin = function () { return CFG.checkOrigin !== false && (getOrigin() === 'null'); };
...
function triggerAnyVersionChangeAndBlockedEvents(openConnections, req, oldVersion, newVersion) {
    ...
    return openConnections.reduce(function (promises, entry) {
        ...
        return promises.then(function () {
            ...
            var e = new IDBVersionChangeEvent('versionchange', { oldVersion: oldVersion, newVersion: newVersion });
        ...
        if (!connectionsClosed()) {
            return new SyncPromise(function (resolve) {
                ...
                var e = new IDBVersionChangeEvent('blocked', { oldVersion: oldVersion, newVersion: newVersion });
                setTimeout(function () {
...
function cleanupDatabaseResources(__openDatabase, name, escapedDatabaseName, databaseDeleted, dbError) {
    ...
    if (CFG.deleteDatabaseFiles !== false && ({}.toString.call(process) === '[object process]')) {
        require('fs').unlink(path.join(CFG.databaseBasePath || '', escapedDatabaseName), function (err) {
...
function createSysDB(__openDatabase, success, failure) {
    ...
    if (sysdb) {
        ...
        sysdb = __openDatabase(typeof CFG.memoryDatabase === 'string'
            ? CFG.memoryDatabase
            : path.join((typeof CFG.sysDatabaseBasePath === 'string'
                ? CFG.sysDatabaseBasePath
                : (CFG.databaseBasePath || '')), '__sysdb__' + (CFG.addSQLiteExtension !== false ? '.sqlite' : '')), 1, 'System Database', CFG.DEFAULT_DB_SIZE);
...
IDBFactory.__createInstance = function () {
    function IDBFactory() {
        ...
        this.modules = {
            ...
            Error: Error,
            ShimEvent: ShimEvent,
            ShimCustomEvent: ShimCustomEvent,
            ShimEventTarget: ShimEventTarget,
            ShimDOMException: ShimDOMException,
            ShimDOMStringList: ShimDOMStringList,
            IDBFactory: IDBFactoryAlias
        };
        this.utils = { createDOMException: createDOMException }; // Expose for ease in simulating such exceptions during testing
        this.__connections = {};
    }
...
IDBFactory.prototype.open = function (name /* , version */) {
    ...
    function dbCreateError(tx, err) {
        ...
        var evt = createEvent('error', err, { bubbles: true, cancelable: true });
    ...
    function openDB(oldVersion) {
        ...
        if (oldVersion > version) {
            var err_1 = createDOMException('VersionError', 'An attempt was made to open a database using a lower version than the existing version.', version);
            if (useDatabaseCache) {
                setTimeout(function () {
                    dbCreateError(err_1);
                });
...
IDBFactory.prototype.webkitGetDatabaseNames = function () {
    ...
    function dbGetDatabaseNamesError(tx, err) {
        ...
        calledDbCreateError = true;
        // Re: why bubbling here (and how cancelable is only really relevant for `window.onerror`) see: https://github.com/w3c/IndexedDB/issues/86
        var evt = createEvent('error', err, { bubbles: true, cancelable: true }); // http://stackoverflow.com/questions/40165909/to-where-do-idbopendbrequest-error-events-bubble-up/40181108#40181108
        req.__readyState = 'done';
        req.__error = err;
        req.__result = undefined; // Must be undefined if an error per `result` getter
        req.dispatchEvent(evt);
    }
    var req = IDBRequest.__createInstance();
    createSysDB(me.__openDatabase, function () {
        sysdb.readTransaction(function (sysReadTx) {
            sysReadTx.executeSql('SELECT "name" FROM dbVersions', [], function (sysReadTx, data) {
                var dbNames = ShimDOMStringList.__createInstance();
